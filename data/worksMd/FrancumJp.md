...
:FileCheck
:: ローカルのアプリケーションデータまたはプロジェクトディレクトリ内にある既存の依存関係を検出する
...
:SetupProject
:: 隔離されたPowerShellスクリプトを介して、コアエンジンの環境セットアップを制御する
...
    glBindBufferBase(GL_UNIFORM_BUFFER, 2, CamUBOID); // ユニフォームスロット2にバインド
