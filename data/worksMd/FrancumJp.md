...
制作者：マリアーニ フランチェスコ パオロ

**ソースコード:** https://github.com/Kiliken/FrancumEngine

**個人サイト:** https://kiliken.github.io/index.html?lang=jp

> 📌 **採用担当者様へ**
> 
>本PDF資料は個人サイト上でも公開しており、Webサイト版ではデモ動画の即時再生、および他のプロジェクトの資料もご確認いただけます。大変恐縮ですが、審査の際は併せてご活用いただけますと幸いです。
...
**デモ動画:** https://www.youtube.com/watch?v=2Jgt69h8ZdM

| ![BaseDemo](data/demo.jpg) | ![GraphicDemo](data/graphic.jpg) |
| :--: | :--: |
| ![GameDemo](data/game.jpg) | ![RandomDemo](data/random.jpg) |
...
> 💡 **各プロジェクトの詳細について**
>
> 私の個人ウェブサイト（冒頭のリンク）では、以下の各作品について、本エンジン（Francum Engine）と同様に詳細な技術解説、設計アーキテクチャ、および実機デモ動画を掲載した「個別紹介ページ」を設けております。併せてご高覧いただけますと幸いです。

| **作品** | **開発環境 & 説明** |
|:-:|:-:|
| **Sin No Hate** <br> <img src="img\showcase\sinNoHate.jpg"> | *OpenSiv3D* <br><br>  **罪の果て**は、**OpenSiv3D**を用いて開発したハイスピード・トップダウン・アーケードシューティングゲームです。**Bandai Namco Studio Cup \| Siv3D Game Jam 2025**参加作品として制作され、プレイヤーは押し寄せる敵を倒しながら「罪（Sins）」を浄化し、最後のボスを撃破して翼を取り戻し、地獄からの脱出を目指します。本作では、カスタムゲームループ、GPUによるパレットスワップシェーダー、最適化された衝突判定・レンダリングシステム、そして軽量なプロシージャルパーティクルエフェクトを実装し、高速かつ爽快な2Dアクションを実現しました。 |
| **Psychic Showdown** <br> <img src="img\showcase\psyShow.jpg"> | *Unity & .NET* <br><br> **Psychic Showdown** は、**東京ゲームショウ2025**に出展した**Unity**製の3Dオンライン対戦アクションゲームです。プロシージャル生成されるステージを舞台に、超能力とアイテムを駆使した対戦を楽しめる作品で、私はネットワークシステムおよびプロシージャル生成の実装を担当しました。 |
| **Reikai Revenge** <br> <img src="img\showcase\tatakae.jpg"> | *Unity & Apache* <br><br> **霊界リベンジ**は、Unityで開発されたPC・Android向けのオンライン対戦ゲームです。プレイヤーは復讐心を抱く霊となり、能力を選択してオンラインロビーを作成・参加し、友人とリアルタイムで対戦できます。対戦では、相手は攻撃を命中させるまで姿が見えないため、戦略性の高いゲームプレイが特徴です。私は、オンライン対戦を支える**PHPサーバーの開発**と、**Unity Web APIを用いたサーバーとの通信機能の実装**を担当しました。 |
| **Direct2D Raycaster** <br> <img src="img\showcase\d2d1fps.jpg"> | *Direct2D & SDL3* <br><br> **Direct2D Raycaster**は、**Wolfenstein 3D**のような初期の3Dゲームで用いられたクラシックなレイキャスティング技術を学ぶための、小規模な教育用プロジェクトです。描画には **Microsoft Direct2D**、ウィンドウの作成と入力処理には **SDL3** を使用し、2DグラフィックスAPIのみを用いて擬似3D空間を描画する仕組みを実装しています。 |
