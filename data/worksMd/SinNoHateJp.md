...
本モジュールは、アクティブなエンティティのリアルタイムなフレーム更新、衝突判定、およびレンダリングパスを制御するコアシステムです。プレイヤーの弾と敵との間で、ネストされた境界ボックス（Bounding Box）による交差判定を処理します。また、標準ライブラリのイテレータ削除パターン（`enemies.erase()`）を用いて構造的な状態変化を安全に管理し、レンダリングパイプラインを構築します。さらに、アクティブなスプライトレイヤーを描画する前に、グラフィックスパイプラインでパレットスワップ（色置換）のシェーダーパスを実行できるよう、カスタム定数バッファとテクスチャレジスタをGPUへ明示的にバインドしています。
...
テクスチャメモリのオーバーヘッドを最小限に抑えつつ、視覚的なバリエーションを最大化するために設計したカスタムHLSLピクセルシェーダーです。敵のバリエーションごとに個別のスプライトシートを保持するのではなく、ベースとなるスプライトをサンプリングし、動的な2Dテクスチャのルックアップテーブル（LUT）を参照する手法を採用しました。ベーステクスチャの画素とソースインデックスとの色距離（カラーディスタンス）を判定することで、定数バッファ（`b1`）経由で渡された選択インデックスに基づき、実行時に色をターゲットのパレット行へと動的に置き換えています。
...
多態性（ポリモーフィズム）を持つインタフェース（`IEffect`）から派生した、軽量な視覚効果（VFX）構造体のコレクションです。フルスケールの物理エンジンによるオーバーヘッドを避け、カスタム物理シミュレーションとプロシージャルな数式を用いることで、動的な2D演出を実現しています。
* **`Spark`:** テクスチャスライスに対してカスタムの重力ベクトルと回転速度を適用し、爆発による破片の飛散をシミュレートします。
* **`RingEffect`:** 指数関数的なイージング関数（`EaseOutExpo`）を利用して時間の経過とともにフレームの半径を滑らかに補間し、爆発の衝撃波を表現します。
* **`Feathers`:** 三角関数のサイン（正弦）関数を用いて、模擬重力下でパーティクルが落下する際の横揺れを計算し、有機的な空気抵抗と空気力学的ドラッグ（抗力）を再現します。
